       ...
       {           
           ...
           {               
               ...
               string loginPage = PublicLogic.WebHelper.getPageContentDynamic("http://www.thisdayonline.info/login.php",
                   "textfield=oluwasao&textfield2=jordan23&submit=login", "cookies",UnicodeEncoding.UTF8);
               #endregion
               string mainUrl = ConfigurationManager.AppSettings[key];
               string mainPage = PublicLogic.WebHelper.Sanitize(
                   PublicLogic.WebHelper.getPageContent(string.Format("{0}/news.php?theme=64", mainUrl), Encoding.UTF8)
                   );
                   ...
                   if (Regex.IsMatch(linksMain, @"<li><a\s*href\=[""'](?<link>.+?)[""']\>", _regOptions))
                   {
                       foreach (Match m in Regex.Matches(linksMain, @"<li><a\s*href\=[""'](?<link>.+?)[""']\>", _regOptions))
                       {
                           ...
                           string feed = PublicLogic.WebHelper.Sanitize
                               (
                               PublicLogic.WebHelper.getPageContent(string.Format("{0}{1}", mainUrl, m.Groups["link"].Value), Encoding.UTF8)
                               );
                           //strip the data
                           //create a file
           ...
           {               
               ...
               string filePath = "C:\\test\\NewsFeedErrors.txt";

               if (!File.Exists(filePath))
                   File.Create(filePath);

               File.WriteAllText(filePath, string.Concat(ex.Message, ex.InnerException,  ex.StackTrace));
