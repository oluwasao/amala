    ...
    {        
        ...
        public void FtpFile(string content, string fileName)
        {
            if (!string.IsNullOrEmpty(ftpLocation))
                //send to ftp server
                PublicLogic.WebHelper.FtpFileFromMemoryStream(fileName, content, ftpLocation, UserName, Password);
            if (!string.IsNullOrEmpty(FtpFileArchivePath))
                //send to another folder
                PublicLogic.WebHelper.FtpFileFromMemoryStream(fileName, content, FtpFileArchivePath, UserName, Password);
